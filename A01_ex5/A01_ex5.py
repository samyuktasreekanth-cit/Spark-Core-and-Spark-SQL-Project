...
def my_main(sc,
    ...
    #Filter the data - Filter if the bus is at stop or not
    filterRDD = linesRDD.filter(lambda x : x[9] == 1)

    # Group data by bus Line
    # (bus_line , (delay, 1))
    mappedRDD = filterRDD.map(lambda x : (x[1], (x[6], 1)))

    # reducebykey key:Bus_line where you get the (sum_delays, sum_counts of how many times that bus_line has occured)
    reducedRDD = mappedRDD.reduceByKey(lambda x, y: (abs(x[0]) + abs(y[0]), x[1] + y[1]))

    # Average delay for each bus line
    averageDelayRDD = reducedRDD.mapValues(lambda x : x[0] / x[1])

    # Sort by the increasing order of average delay 
    solutionRDD = averageDelayRDD.sortBy(lambda x : x[1])
